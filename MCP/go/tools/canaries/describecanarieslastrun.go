...
func DescribecanarieslastrunHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/canaries/last-run%s", cfg.BaseURL, queryString)
		...
		var result models.DescribeCanariesLastRunResponse
...
func CreateDescribecanarieslastrunTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_canaries_last-run",
		...
		mcp.WithString("NextToken", mcp.Description("Input parameter: A token that indicates that there is more data available. You can use this token in a subsequent <code>DescribeCanariesLastRun</code> operation to retrieve the next set of results.")),
		mcp.WithNumber("MaxResults", mcp.Description("Input parameter: Specify this parameter to limit how many runs are returned each time you use the <code>DescribeLastRun</code> operation. If you omit this parameter, the default of 100 is used.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DescribecanarieslastrunHandler(cfg),
	}
}
